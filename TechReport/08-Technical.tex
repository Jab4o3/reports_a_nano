...
Taking the general-equation poles and their counterparts from the $Z_t$ expression yields the system of equations shown in Equation \eqref{eq:td:poles}. Solving for one pole first results in a quadratic equation with roots shown in Equation \eqref{eq:td:pole_roots}.
...
Ultimately, the charge pump is the most fit for this project, because it is relatively simple, while also offering excellent stability. The decision was reached only after taking into account the power calculations from Chapter \ref{chap:fd:power_req}. according to which all devices can handle the power requirements of the circuit. 
