...
The most interesting metric for data acquisition is the maximum sampling frequency before data loss occurs, as clear results require it to be much higher than the signal frequency. According to Nyquist's theorem, to sample a sinusoidal signal with a frequency of $f_{sig}$ the sampling frequency $f_s$ needs to be $f_s > 2f_{sig}$. However, square waves are comprised of multiple harmonics, in addition to the fundamental of the signal, which necessitates a drastic increase in $f_s$. In fact, to sample a useful amount of harmonics\footnote{Square waves contain so-called odd harmonics, which are harmonics at frequencies that are multiples of the fundamental and odd integer series. Minimum square-wave definition can be had with only up to the second harmonic (with a frequency of $5f_{sig}$), but adapting sampling for up to the twelfth harmonic (with a frequency of $25f_{sig}$) was chosen to improve measurement accuracy}, sampling should be done with a frequency $f_s > 2f_{harm5} = 50f_{sig}$. Lastly, because the measured signal is not periodic, the lowest pulse duration, which is the minimum dark time, needs to be taken to calculate $f_{sig}$. As a result the sampling frequency needs to be set to $f_s > \frac{50}{\tau_{dmin}}$.
...
Generating pulsing sequences can be achieved with dark times in the required range of 100 \unit{\nano\second} to 10 \unit{\micro\second} with little issue. Additionally, the part-linear, part-logarithmic function that is used for spacing the dark times, so that there are more data points at lower values of $\tau_{d}$, functions as expected, which was clearly demonstrated in Figure \ref{fig:td:logspace}.
