\chapter{Photodetector design}
As an integral part of the sensing setup, the photodetector needs a lot of attention. Good amplification is crucial for increasing the \gls{snr} of the quantum setup, however the materials and components used need to be accessible and affordable. Based on these considerations and the design ideas presented in the functional design (see Chapter \ref{chap:photodetection_design}), the photodetection circuit can be drawn up. This chapter covers concepts relevant for both \gls{cwodmr} and \gls{podmr}[] photodetection. 

To summarize the circuit requirements, the input signal of the diode can range from \numrange{0}{50} \unit{\milli\ampere}. This current needs to be amplified in the range from \numrange{0}{5} \unit{\volt}. In addition, the circuit should consider the quantum protocols that it will be used to detect. For \gls{cwodmr}, low noise levels at low frequencies are desired and a bandwidth of 18 \unit{\hertz} is sufficient. In terms of pulsed protocols, Rabi oscillations is the one that has the most bearing on the requirements. As previously mentioned the \gls{podmr}[] detector bandwidth of at least 5 \unit{\mega\hertz} was determined based on protocol specifications, as well as research \cite{sewani2020coherent, nandi2024oscilloscope, murzakhanov2025mechanism}. Noise is harder to minimize with the broader bandwidth, which is why it was decided to have separate detectors for \gls{podmr} and \gls{cwodmr}.

\section{Proposed design}\label{chap:td:prop_design}
As previously mentioned, the first implementation of the photodetection \gls{pcb}, also referred to as "v1", is based on a proposed design, provided by a collaborator of the research group. Although its main purpose is to enable the setup to do \gls{cwodmr} measurements, it also provides a reference for the following designs, which optimize the detector for the sensing setup and explore pulsing protocol support. In addition, the design was implemented so that its price is as low as possible, at the request of the client.

The collaborator only provided a simulation with no specific information for the design process, aside from noting the photodetector was made for input currents in the range of \numrange{0}{50} \unit{\nano\ampere}. Such low inputs require high gain-setting resistor $R_1$ values, however increasing the resistor value results in more biasing current going to the amplifier and a more limited dynamic range \cite{black2021tia}. Furthermore, it is entirely possible that the original design did not thoroughly consider the bandwidth and noise levels needed for \gls{cwodmr} setups. 
...
\section{Frequency and noise analysis}\label{chap:td:calc:v2}
...
This section uses the mathematical principles presented in \cite{margan2012transimpedance, sackinger2017analysis, horsthemke2025all} to optimize the proposed \gls{tia} design. It presents an analytical approach to designing a \gls{tia}, taking into account the general practices, but also considering more subtle factors that might hinder the performance of the system and increase the noise.
...
Introducing the real-world parasitic capacitances to the ideal \gls{tia} shown in Figure \ref{fig:tia} results in the circuit in Figure \ref{fig:td:tia_circuit_parasitic_v1}. The diode capacitance $C_d$, combined with the differential and common-mode capacitances of the amplifier ($C_{df}$ and $C_{cm}$), contributes a significant amount of capacitance to the circuit and can result in instability if not addressed \cite{cherian2016you}.

\begin{equation}\label{eq:td:ci}
	C_i = C_{d} + C_{df} + C_{cm}
\end{equation}

To verify that a design is stable a formula derived from the rate of closure of the system can be used \cite{ti2019simplify} (see Equation \eqref{eq:td:tia_stability}).

\begin{equation}\label{eq:td:tia_stability}
	GBW > \frac{C_i + C_1}{2\pi R_1C_1^2}
\end{equation}
...
\begin{equation}\label{eq:td:kcl}
	i_f = \frac{v_in}{\frac{1}{sC_i}} + \frac{v_{in} - v_{out}}{Z_1(s)}
\end{equation}
...
Before moving on to the pole analysis, the impact of the quality factor $Q$\footnote{The quality factor of a system describes how dampened a system is. If $Q = 0,5$, the system is critically damped. If $Q > 0,5$ the system is underdamped and can present unwanted effects like ringing and peaking. If $Q < 0,5$ the system is overdamped, which results in a reduced bandwidth.} on the system response should be considered (see Equation \eqref{eq:td:f_c_q}). Two system poles present when $Q > 0,5$. For $Q = \frac{\sqrt{3}}{3},$ the system has a Bessel response, which has the flattest group delay, however, if $Q = \frac{\sqrt{2}}{2}$, then the system has a Butterworth response, which means it has the flattest possible amplitude curve. Although the group delay is not as important as the amplitude response, a Bessel response also results in smaller amount of overshoot, less jitter and less noise overall, which is why it is preferred. In the end, the choice between a Bessel and a Butterworth response only affects the circuit at high frequencies, which is not a concern when running \gls{cwodmr} measurements. Still, it is important when investigating photodetection for pulsed protocols (see Chapter \ref{chap:td:podmr}). The broad bandwidth of such a photodetector causes the effects of the system response to manifest much more clearly than in low-frequency \glspl{tia}. An overdamped system loses a significant amount of bandwidth and an underdamped system exhibits significantly more high-frequency noise. For this reason, a Bessel response is generally preferred, but the system can be tuned by increasing the value of $C_1$ slightly, causing a decrease in noise, but also a shrinking of the signal bandwidth.

\begin{equation}\label{eq:td:poles_general}
	H(s) = H_0\frac{(-s_1)(-s_2)}{(s - s_1)(s - s_2)} = H_0\frac{s_1s_2}{s^2 + s(-s_1 - s_2) + s_1s_2}
\end{equation}

Based on this consideration, the poles can be found using the general form of a second order transfer function, shown in Equation \eqref{eq:td:poles_general}. The general form also makes it obvious that the \gls{dc} gain $H_0 \approx R_1$, assuming that $\frac{A_0}{A_0 + 1} \approx 1$.
...
Achieving the desired system, can be done by getting the roots of the general form of a Bessel filter (as seen in Equation \eqref{eq:td:pole_roots_general}) and using the imaginary parts to get Equation \eqref{eq:td:cap_solve}. From it, a the quadratic equation in Equation \eqref{eq:td:cap_quad} can be derived. This expression can be solved with the quadratic formula again, finally resulting in an answer for the compensating capacitor $C_1$. 
...
To further characterize the system, the cutoff frequency $\omega_c$ and quality factor $Q$ can be calculated, because the system adheres to the general form, shown in Equation \eqref{eq:td:f_c_q}. Substituting in the pole expressions from Equations \eqref{eq:td:poles_general} and \eqref{eq:td:poles}.
...
The noise gain is the ratio of the output voltage $v_{out}$ and the total internal noise at the input $v_n$. This ratio gives a better understanding of what frequencies of noise are the most problematic if they appear at the input of the \gls{tia}. In order to get a better understanding of the noise gain, its transfer function can be calculated in much the same way as the transimpedance (Equations \ref{eq:td:aol} - \ref{eq:td:zt}). Equation \eqref{eq:td:noise_vin} shows how the input voltage $v_{in}$ can be expressed with the inverting input voltage of the amplifier $v_{ia}$ and the equivalent noise source $v_n$. The equation also shows the difference between the signal and noise transfer functions. The latter is an abstraction, which treats the system noise without accounting for the photodiode current $i_{f}$, thus providing an alternative way of viewing the system.

\begin{equation}\label{eq:td:noise_vin}
	v_{in}  = v_{ia} + v_n = \frac{v_{out}}{A_o\frac{\omega_o}{s + \omega_o}} + v_n
\end{equation}

\begin{equation}\label{eq:td:noise_kcl}
	\frac{v_{in}}{\frac{1}{sC_i}} = \frac{v_{out} - v_{in}}{Z_1(s)}
\end{equation}

Summing the currents at $v_{in}$ (see Equation \eqref{eq:td:noise_kcl}) leads to Equation \eqref{eq:td:noise_tf}. The transfer function shown in it can be broken down into two components: a low-pass and a band-pass component. In the numerator, expression $sA_o\omega_o$ contributes the band-pass component and the expression $\frac{A_o\omega_o}{(C_i + C_1)R_1}$ is responsible for the low-pass component. Figure \ref{fig:td:noiseexample} shows an example of the frequency response of the noise transfer function.

\begin{equation}\label{eq:td:noise_tf}
	H_n(s) = \frac{v_{out}}{v_n} = \frac{A_o\omega_o(s + \frac{1}{(C_i + C_1)R_1})}{s^2 + s(\frac{1}{(C_i + C_1)R_1} + \omega_o(1 + A_o\frac{C_1}{C_i + C_1})) + \frac{\omega_o(1 + A_o)}{(C_i + C_1)R_1}}
\end{equation}


\begin{figure}[ht]
	\centering
	\includegraphics[width=0.8\linewidth]{matlab/noise_example}
	\caption{Plot of the components of the noise transfer function (using the AD795 op-amp; $R_1$ = 10 \unit{\mega\ohm} and $C_1$ = 1,6 \unit{\pico\farad})}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.8\linewidth]{matlab/noise_example_vs_signal}
...
Lastly, it should be mentioned that the peak in the noise transfer function is reflected in the spectral noise density when the system has a Bessel response. To attenuate the noise peak, the capacitance of $C_1$ can be increased, which also leads to a decrease in signal bandwidth. This adjustment of $C_1$ serves as one of the key differences between the two photodetectors, because the \gls{cwodmr}[] circuit can work even with an overdamped response, so the compensation capacitor is less important and can be chosen based on what standard values are on offer. On the contrary, the \gls{podmr} photodetector requires a much broader bandwidth, so the adjustment from a perfect Bessel response should be minimal.


\section{Power supply and conversion}\label{chap:td:power}
The most important considerations for the setup are function and size. Form factor constraints arise from the fact that the setup needs to be as small and adhere to the Thorlabs photonics mounting standard. Photodetection also needs to not require many external connections or ideally be standalone. This is feasible for the data flow, as the photodetection \gls{pcb} only needs light and an output to work. Unlike the data connections, version two of the design has a suboptimal power delivery system, as it uses three pins for the supply of power. 
...
\section{Bandwidth expansion}\label{chap:td:v4_design}
\begin{figure}[ht]
	\centering
...
It is possible to simplify the formula, because when designing the amplifier, the ratio of the resistors is fixed ($R_3 = A_rR_2$, where $A_r$ is the resistor ratio). Solving for $R_2$ then gives Equation \eqref{eq:td:v4:nia_cascading:r2_simplified}.
...
\chapter{Photodetector simulation}
While the previous sections covered design choices based on general \gls{tia}[] development techniques, this section explicitly covers the simulation and implementation of \glspl{pcb}.

\section{First iteration}
This version of the photodetector (v1) is a direct implementation of the proposed design discussed in Chapter \ref{chap:td:prop_design}, with only slight modifications and additions where the collaborator did not explicitly define design specifications.
...
Figure \ref{fig:photodetecog} shows the design schematic. The capacitor $C_4$, as well as the current source $I_1$ are used to simulate the behavior of a photodiode.
...
\section{Second iteration}\label{chap:td:v2}
The second iteration (v2) of the photodetector design is based on the system analysis in Chapter \ref{chap:td:calc:v2}. It is designed to have a Bessel response, the compensation capacitor for which was calculated by a MATLAB script.


MATLAB calculates the component values with high precision and sourcing real components that precisely match the values is not possible, which is why components with standard values will be used in the setup. Simulations with standardized components were also done to compare the mathematically-ideal setup to the one in practice. 
...
While the second version of the photodetector focuses on improving the TIA performance, the third iteration (v3) is more about adapting the design for the specifics of the setup. In particular, this iteration tries to simplify the power delivery and optimize the size of the design, while keeping the amplifying circuit unchanged. In spite of the added power management, this version of the system is made so that it still costs around the same as the previous ones.
...
\section{Fourth iteration}\label{chap:td:v4_sim}
Although the previous iterations provided an inexpensive and reliable photodetector for \gls{cwodmr}, the narrow bandwidth prevents them from measuring pulsed protocols. This iteration (v4) is specifically designed for $T_1$ measurement support. Although it is somewhat more expensive than the previous iteration, version four justifies the price by offering better sensing capabilities. Although the design can only handle low-frequency $T_1$ measurements, it serves as proof of concept for the making of a device that can handle Rabi oscillations, as well.
...
As previously discussed, the value of the capacitor could not be determined analytically, however an approximate value was calculated using MATLAB simulations of the mathematical system model. Figure \ref{fig:td:bodebootstrapvsnostrap} shows that the bootstrapped system demonstrates a significantly increased bandwidth. According to Table \ref{tab:td:v4_params}, the bootstrapped amplifier still has a quality factor $Q$ close to the ideal \num{0,57735}, which is even better than the previous iterations of the system. Furthermore, the capacitor value is standard, meaning that the value can be used in the physical system without further approximations.
...
\section{Final \glsfmtshort{cwodmr} version}\label{chap:td:v5_design}
At the beginning of the project, the client wanted an all-encompassing solution for the detection of all protocols. Although this is possible, as is clear from the photodetectors on the market, such a solution can cost in the range of hundreds to thousands of euros. Bringing costs down is important for the client, which is why it was eventually decided that separate photodetectors for \gls{cwodmr} and \gls{podmr} were needed, as is stated in the requirements. 
...
\section{Final \glsfmtshort{podmr} version}\label{chap:td:podmr}
Based on the previously discussed possible bandwidth improvements, a new design was drawn up. Unfortunately, the fourth iteration with the bootstrapped photodetector had biasing issues that could not be resolved, which was found out when doing the tests in Chapter \ref{chap:test:v4_results}. Consequently, the final \gls{podmr}[] design does have a bootstrapped photodetector. The new iteration was made with Rabi oscillations in mind, which occur at over 3,2 \unit{\mega\hertz}. Increasing the bandwidth to 5 \unit[]{\mega\hertz} to support the Rabi oscillations protocol was the primary consideration when designing the \gls{pcb}. This photodetector is based on the TEMD1000 photodiode, because of its low capacitance, which contributes to a much broader bandwidth. Additionally, the \gls{tia} uses the OPA858 op-amp, because of its high \gls{gbp} and low parasitic capacitance. As this design has not been fully implemented, the recommendations chapter (Chapter \ref{chap:recs:photodetector}) discusses some of the underlying design choices that might be subject to change in the future.
...
It should be noted that cascading two post-amplification stages is a viable alternative, as it would result in a 30\% bandwidth increase with the previously mentioned specifications. This layout also results only 20\% more noise. Figure \ref{fig:recs:v6_sim} shows the frequency response of the system, as well as the noise plot. The cutoff frequency of the detector is 9,3 \unit{\mega\hertz}, around which the noise spikes to 31,72 \unit{\micro\volt\per\hertz\tothe{0.5}}. For most of the bandwidth, though, the noise is \qty{12,87}{\micro\volt\per\hertz\tothe{0.5}}. Increasing the capacitance to 30 \unit[]{\femto\farad} results in a bandwidth of \num{5,76} \unit[]{\mega\hertz}, which is just above the minimum of 5 \unit[]{\mega\hertz}. This decreases the noise spike to \num{21,44} \unit{\micro\volt\per\hertz\tothe{0.5}}.

\chapter{Photodetector implementation}
This chapter deals with the photodetection \glspl{pcb}, which are based on the designs and simulations discussed in the previous chapters.
...
The TPS60400 charge pump was selected for use in the photodetector in order to invert the 5 \unit{\volt} power supplied to the board by an external supply. The charge pump, combined with the fixed board dimensions, made it more difficult to do the placing and routing. Additionally, the clearance of the power and ground nets was increased. Figure \ref{fig:td:imp:photodetecv3} shows the \gls{ecad} view of the third iteration of the photodetector. 
...
\section{Final \glsfmtshort{podmr} version}
...
Due to the long shipping times and limited lab access near the end of the project, this design has not been fully implemented and is only a proposal for future reference. A physical implementation of the Kicad design has not been realized and tested, so the board might need modifications in order to work. In addition, the behavior of the photodiode has only been estimated, as the current generation of the device could not be tested when exposed to the green laser. The long shipping times were the main reason why the characterization of the TEMD1000 diode could not be done.
