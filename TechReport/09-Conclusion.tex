...
It is important to mention, before delving into testing details, that as of completing this report, the hardware setup for $T_1$ measurements has still not been completed. This limit on the implementation and testing possibilities made it impossible to complete the software, which is why only certain unit tests could be done. Furthermore, testing was hampered by hardware unavailability and limited lab access.

Aside from proper functioning, the client set no requirements for speed. This is because the application relies on several fixed-length delay in order to operate. Because of this, all subsystems were tested and compared to the expected behavior with no regard for the length of the execution time.
...
\subsection{\glsfmtshort{daq} test setup}
...
\subsection{\glsfmtshort{daq} test}
Data acquisition
