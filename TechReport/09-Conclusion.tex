...
Finally, the full setup diagram can be seen in Figure \ref{fig:test:tia_dc:setup}. Using the function generator feature of the Analog Discovery 2, a voltage can be generated and, using the current through $R_d$, the oscilloscope output can be sued to characterize the system.
...
For the tests, it was decided to test the \gls{dc} characteristics of the amplifier separately from the frequency response. To verify the amplification matches the simulations, a low-frequency pulse is used and its amplitude is varied, all while the output voltage is measured. The frequency response test uses pulses at various frequencies, similarly to the integration test. The difference is that instead of pulsing the laser, the pulses generate current using the photodiode model, which is then amplified as voltage.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.7\linewidth]{img/setup_model_alt}
...
\begin{figure}[!ht]
	...
	\begin{subfigure}[1a]{.49\linewidth}
		\centering
		\includegraphics[width=\linewidth]{img/v2_model_5nA}
	...
	\begin{subfigure}[2a]{.49\linewidth}
		\centering
		\includegraphics[width=\linewidth]{img/v2_model_25nA_alt}
	...
	\begin{subfigure}[2b]{.49\linewidth}
		...
		\caption{Model test with the input at 50 \unit{\milli\volt} and a 50 \unit{\nano\ampere} current through $R_d$ (\num{4,4}\unit{\volt} in simulation)}
		\label{fig:test:v2_model_40nA}
	\end{subfigure}
	
	\caption{Test with a photodiode model ($R_d$ = 1 \unit{\mega\ohm}, $C_d$ = 27 \unit{\pico\farad}) of the second iteration of the photodetector with a 50 \unit{\hertz} input signal}
	\label{fig:test:v2_model}
\end{figure}


Figure \ref{fig:test:v2_model} shows the results of the \gls{dc} characteristics tests. Despite the 5 and 15 \unit{\nano\ampere} measurements showing results similar to the simulations, the output signal is not very stable and contains significant noise. At higher currents, the distortion of the signal is much less pronounced, but there is still a significant amount of noise.

To gain a better understanding of the noise, it was measured with the diode model connected, but turned off. The results are shown in Figure \ref{fig:test:v2modelnoise} and show sporadic changes in voltage with no discernible periodic component, which means the noise is most likely caused by the diode model and significantly amplified by the amplifiers. 


\begin{figure}[!ht]
	\centering
	\includegraphics[width=0.7\linewidth]{img/v2_model_noise}
	\caption{Noise of the second iteration when connected to the diode model}
	\label{fig:test:v2modelnoise}
\end{figure}


Figure \ref{fig:test:v3_model} shows the 
\begin{figure}[ht]
	\centering
	\begin{subfigure}[1a]{.49\linewidth}
		\centering
	...
	\caption{Test with a photodiode model ($R_d$ = 1 \unit{\mega\ohm}, $C_d$ = 27 \unit{\pico\farad}) of the third iteration of the photodetector with a 50 \unit{\hertz} input signal}
...
There are many factors that affect the measurements, but perhaps most prominent are the environmental lighting sources. This includes ambient sunlight, which gets through the blinds of the lab, and reflections from the laser. Both significantly affect the photodetector, especially when there is no focused light present. Reflections, in particular, majorly contributed to signal distortion at higher frequencies
...
Firstly, increasing the photodetector bandwidth should be the primary concern of the people working on the setup integration, as no version of the detector can measure Rabi oscillations, which occur at over 3 \unit{\mega\hertz}. Version four of the photodetector has a much wider bandwidth, but it is bottlenecked by the photodiode, which is only stable at up to 350 \unit{\kilo\hertz} in the photovoltaic configuration \cite{photodiodephotodiode}. However, biasing a BPW34 diode in photoconductive mode will result in \numrange{2}{30} \unit{\nano\ampere} of dark current, rendering the actual signal unreadable. Alternatively, the gain can be increased by finding op-amps with bigger \gls{gbp}.
%todo:explain opamp change
