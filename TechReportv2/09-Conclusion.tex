...
After covering the photodetection \gls{pcb} iteration that were tested, it is important to mention the ones that were not. The final \gls{cwodmr} design was implemented, however it could not be assembled and tested due to time constraints. All preliminary iteration should perform \gls{cwodmr} with no issues, so its implementation is not mandatory. Still, the final design performs much better when modeled mathematically and in simulations. Additionally, a \gls{podmr} design was proposed based on calculations and simulations. It should handle both $T_1$ measurements and Rabi oscillations, but it needs to be implemented first.

\subsection{$T_1$ software}
The software for generating pulsing sequences and acquiring $T_1$ measurements results works as intended. 
...
According to the introduction in Chapter \ref{chap:intro}, the main purpose of the project was create a photodetector, but making a sequence generation and data acquisition software and integrating the hardware and software with the rest of the sensing setup was also important. All original goals were tackled during this project, with the focus being on photodetection. \gls{cwodmr} photodetection was achieved and a viable solution for \gls{podmr} detection was proposed. The software was almost completed, with only a decoding module to process the acquired data missing. Further refinements to the photodetector and the setup as a whole are possible. Some of the most promising ones are discussed in the recommendations in Chapter \ref{chap:recs}. Complete integration of the whole setup necessitates more work, but is achievable in the near future.
...
The final \gls{cwodmr} version, also known as the fifth photodetector iteration, performs very well according to the calculations and simulations, but, as previously mentioned, it could not be tested in time. The circuit design has been finalized, however, so future contributors can simply continue by assembling and testing it. 

Unlike version 6, the final \gls{podmr} version has only been designed and simulated. Proper implementation and testing needs to be done before it can be put into practice. That being said, the design shows promise and, if it performs as expected, it will support both $T_1$ measurements and Rabi oscillations. 
...
Following the test result discussion and conclusion, this chapter discusses what improvements can be made to the products of the project. It is separated in two sections, which cover the photodetection platform and the software application for $T_1$ measurements. The photodetector section provides an overview of bandwidth improvements, specifically focusing on \gls{podmr}[] photodetection. The section on the $T_1$ application discusses the functionality that needs to be implemented to finalize the data processing. In addition, the section goes over possible improvements that, if implemented, could improve the performance and stability of the application. At the end of the section, some notes on hardware are also included, as it limits what the application can do. The section on lock-in amplification is an extension of the software section, as it covers data acquisition by using the \gls{lia} output (and consequently \gls{api}) directly.
...
Due to the relative simplicity of simply omitting the secondary output device (previously the \gls{ad2}[]), this method for solving the sampling rate issues of the \gls{daq} system is recommended. Furthermore, this section demonstrates that the software application, just like the physical setup, will be simplified if only a \gls{lia} is used as a \gls{daq}.