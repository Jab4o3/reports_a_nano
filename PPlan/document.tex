...
\begin{document}
	...
	\textbf{Task 1.1} is to build an OLIA amplifier, which can be used at Applied Nanotechnology's laboratory. This will be done with the technical specifications and firmware provided by Harvie and de Mello \cite{harvie2023olia}. % TODO: add more specificity after discussion
	
	\textbf{Task 1.2} is the follow-up to \textbf{task 1.1}. It involves the design and production of a photodiode PCB. The PCB has to output signals that are not only compatible with the OLIA, but also with lock-in amplifiers that are available on the market. % TODO: add more specificity after discussion
	
	\textbf{Task 2.1} is to write a application in Python, MATLAB or alternatively create an HTML-based web application. This can be done on a different setup, but ideally it will use the hardware setup from \textbf{goal 1}. % TODO: add more specificity after discussion
	
	%\textbf{Task 2.2} 
	
	\textbf{Task 3.1} follows after all previous tasks. The completed setup needs to be used to measure the performance of OLIA and compare it to other lock-in amplifiers available on the market. SNR, bandwidth and stability are the main metrics that need to be compared. % TODO: add more specificity after discussion
	...
	%These deliverables are important for AmI, as they provide all the necessary information about the project itself and how to continue it in the future. 
	
	%Additionally, the last 2 products are also there to ensure the student has completed the learning goals for the project. The final report is important too, as it demonstrates knowledge and the ability to communicate that knowledge. Another important deliverable is the learning report, which is an introspection about the project and its highlights.
