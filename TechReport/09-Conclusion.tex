...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.6\linewidth]{img/setup_integ_alt}
...
The tests of the third iteration of the photodetector, seen in Figure \ref{fig:test:v3_model}, show that the amplifier has a similar performance to the previous iteration. Although there is more noise, the signal is also less distorted. This is not caused by the intrinsic qualities of the two circuits, as they share the same amplification design, but instead is caused by the instability of the model and, more specifically, the connection to the \gls{tia}. Otherwise, both the second and the third version demonstrate the same amplification at all current levels.
...
What is more interesting about the tests of the third version of the photodetector is the noise. Figure \ref{fig:test:v3modelnoise} shows that compared to the previous version, which lacks a charge pump, this iteration has a periodic signal clearly present even when there is no input voltage from the Analog Discovery. Due to the shape and frequency, it can be determined that this is ripple noise from the power supply. Chapter \ref{chap:test:integ_res}

\begin{figure}[!ht]
	\centering
	\includegraphics[width=0.7\linewidth]{img/v3_model_noise}
	\caption{Noise of the second iteration when connected to the diode model}
