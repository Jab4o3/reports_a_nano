...
The fourth photodetector iteration presents a solution specifically designed to handle pulsed protocols. Even though the exact frequency and gain requirements for the circuit are unknown\footnote{Different papers often present different pulsing specifications. In addition, the client wants to adjust the pulsing specifications, so the necessary photodetector bandwidth can only be estimated}, the fourth version is able to do $T_1$ measurements according to the mathematical models, as well as the simulations, however, due to photodiode biasing issues, the circuit outputs smaller signals and also has a smaller bandwidth than previous iterations. As a result, $T_1$ measurements cannot be carried out with it.

Lastly, the software application for doing $T_1$ measurements is mostly completed, with only the signal decoder missing, due to the lack of a finished physical setup. The rest of the code can be used in its current state, but there are some improvements, discussed in Chapter \ref{chap:recs}, that can improve the efficiency and stability of the measurements.
