...
Before moving on to the pole analysis, the impact of the quality factor $Q$ on the system response should be considered (see Equation \eqref{eq:td:f_c_q}). Two system poles present when $Q > 0,5$. For $Q = \frac{\sqrt{3}}{3},$ the system has a Bessel response, which has the flattest group delay, however, if $Q = \frac{\sqrt{2}}{2}$, then the system has a Butterworth response, which means it has the flattest possible amplitude curve. Although the group delay is not as important as the amplitude response, a Bessel response also results in smaller amount of overshoot, less jitter and less noise overall, which is why it is preferred. In the end, the choice between a Bessel and a Butterworth response only affects the circuit at high frequencies, which is not a concern when running \gls{cwodmr} measurements. Still, it will be important when investigating photodetection for pulsed protocols.
...
\section{Software application} \label{chap:td:software}
...
s_available) # get scope channel data and copy it to the buffer
	\end{lstlisting}
