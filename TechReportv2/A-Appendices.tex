...
disp('Sequence period   : ' + T*10^3 + ' ms')
	\end{lstlisting}
	\caption{$T_1$ pulse generation using MATLAB}
	\label{code:t1_pulse}
\end{figure}
...
\begin{figure}[ht]
	\begin{lstlisting}[frame=single,
		numbers=left,
		style=Matlab-Pyglike]
stability_rhs = (C_d + C_cm + C_df + C_1)/(2*pi*R_1*C_1^2);
if (GBW > stability_rhs)
	disp("Stable (" + GBW + " > " + stability_rhs + ")")
else 
	disp("Unstable (" + GBW + " < " + stability_rhs + ")")
end
	\end{lstlisting}
	\caption{\gls{tia} stability calculation}
	\label{code:tia_stability}
\end{figure}
\begin{figure}[ht]
